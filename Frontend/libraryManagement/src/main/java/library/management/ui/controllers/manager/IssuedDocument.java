...
/**
 * The IssuedDocument class offers functionality to manage and display a set of loan instances
 * within a user interface, utilizing the functionalities of JavaFX for dynamic content updates and
 * interactions. It extends the ManagerSubController to gain broader management capabilities
 * alongside the MainController, which it uses for initializing and controlling view components.
 */
public class IssuedDocument extends ManagerSubController {

  /**
   * Represents an observable list of Loan objects within the IssuedDocument class. This list is
   * used to store and manage Loan instances, allowing for dynamic updates and observer
   * notifications. It is initialized as an ObservableList using
  ...
  /**
   * Initializes the issue document view by setting up the cell value factories for various table
   * columns with properties from the loan data model. This method configures the table view to
   * display relevant information about issued documents including loan ID, document ISBN, title,
   * user name, dates of borrowing and due, the number of days for the loan, the fee, and the loan
   * status. Additionally, it initializes the filter combo box for filtering the table view
   * content.
   */
  ...
  /**
   * Handles the search operation based on the selected filter criteria and search text. The method
   * retrieves the selected filter criteria from the user interface and normalizes the search text
   * by trimming and converting it to lowercase. It then clears the current list and performs a
   * search based on the filter criteria:
   * <p>
   ...
   * <p>
   * The search results are then added to the list, which updates the relevant view in the user
   * interface.
   */
  ...
  /**
   * Initializes the filter combo box for the issued document view. This method sets up an
   * observable list with predefined filter options, which include various identifiers, and assigns
   * it to the combo box for issue type selection. The combo box is then set to default to "All ID",
   * providing a comprehensive view of all available options.
   */
   ...
   * <p>
   * Retrieves the list of handled loans from the data access object (DAO) and updates the internal
   * list storage. Subsequently, refreshes the user interface to display the updated loan
   * information.
   * <p>
   * This method clears any existing data in the list before loading new data, ensuring that the
   * most recent and relevant loan information is displayed in the `IDView` associated with the
   * given controller.
   */
