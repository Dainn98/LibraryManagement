...
public class ReturnDocumentController extends ManagerSubController {
   ...
   *
   * @param controller the MainController instance that this ReturnDocumentController will interact with
   */
  ...
  /**
   * Initializes the return document setup by configuring the combo box
   * and setting up a listener for changes in checked items within the issue type combo box.
   * When an item in the combo box is checked or unchecked, the method triggers a search
   * for loans based on the selected criteria.
   *
   * This method relies on the `initComboBox` method to set up the combo box with predefined
   * loan issue types and marks all of them as initially checked. It also registers a listener
   * on the checked items collection of the combo box to invoke the `searchLoanByID` method
  ...
  /**
   * Initializes the issue type combo box within the UI. This method populates
   * the combo box with predefined issue types, which include "pendingReturn",
   * "borrowing", and "late". After adding these items to the combo box, it
   * sets all items as checked by default, allowing initial state selection
   * for the combo box in the UI.
   */
   ...
   *
   * This method clears the current contents of the list and repopulates
   * it with active loan data retrieved from the LoanDAO. It then updates
   * the ListView in the controller with the refreshed list data.
   */
   ...
   *
   * This method processes the return of a selected loaned document.
   * If the document is marked as "late", it checks if a valid late fee is entered.
   * Displays an alert to confirm the return action and another to indicate success or failure.
   *
   ...
   * 3. Displays an alert if the late fee is invalid or prompts confirmation to proceed with return.
   ...
   *
   * This method clears the current list of loans and then repopulates it by querying
   * for loans based on the text input from the searchLoanID field and the checked
   * items in the issueTypeComboBox. The retrieved loans are then set to the listInfo
   * display component.
   */
