...
import library.management.ManagerScreen;
 ...
 * including login, registration, form switching animations, and validation of user inputs.
 * <p>
 * It supports both managers and regular users, providing role-specific access to the application.
 */
public class ModernLoginController extends GeneralController implements Initializable {
  ...
  public void handleLogin(ActionEvent actionEvent) {
    ...
    Object result = authService.signIn(userName, password);

    if (result instanceof Manager manager) {
      openScreen(actionEvent, new ManagerScreen(), manager);
    } else if (result instanceof User user) {
      openScreen(actionEvent, new UserScreen(), user);
    } else {
  ...
  private void openScreen(ActionEvent event, Object screen, Object account) {
    closeCurrentStage(event);

    if (screen instanceof ManagerScreen managerScreen && account instanceof Manager manager) {
      managerScreen.setManager(manager);
      Stage stage = new Stage();
      managerScreen.start(stage);
      System.out.println("Logged in as Manager: " + manager.getManagerName());
    } else if (screen instanceof UserScreen userScreen && account instanceof User user) {
      userScreen.setUser(user);
      Stage stage = new Stage();
      userScreen.start(stage);
      System.out.println("Logged in as User: " + user.getUserName());
    } else {
      throw new IllegalArgumentException("Screen and Account types do not match!");
    }
  }
