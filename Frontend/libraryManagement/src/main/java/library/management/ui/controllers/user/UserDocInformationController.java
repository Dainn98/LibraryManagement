...
/**
 * Controller for managing document information in the user interface. Handles displaying document
 * details, generating QR codes, and managing borrowing and returning actions for documents.
 */
public class UserDocInformationController extends GeneralController implements properties {
  ...
  private void handleBorrow(ActionEvent actionEvent, Document doc) {
    ...
    Optional<ButtonType> result = showAlertConfirmation("Borrowing document",
        "Are you sure you want to borrow this document?");
    if (result.isPresent() && result.get() == ButtonType.OK) {
      Loan loan = new Loan(FullUserController.mainUser.getUserName(), document.getIntDocumentID(),
          number, 0);
  ...
  private void handleReturn(ActionEvent actionEvent) {
    ...
    if (loan.getStatus().equals("late")) {
      double lateFee = Loan.LATE_FEE * ChronoUnit.DAYS.between(loan.getRequiredReturnDate(),
          LocalDateTime.now());
      Optional<ButtonType> result = showAlertConfirmation("Return document",
          "This document is overdue. You have to pay a late fee of " +
              lateFee + " $.\n" +
