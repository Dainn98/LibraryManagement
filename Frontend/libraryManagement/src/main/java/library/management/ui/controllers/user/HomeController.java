...
/**
 * Controller class for the home screen that manages document search, suggestions, and displays
 * documents grouped by categories in a grid view.
 */
public class HomeController extends GeneralController implements SuggestionSearch, properties {
  ...
  /**
   * Initializes the filter options for document search, allowing the user to filter by title,
   * author, publisher, or ISBN.
   */
  private void initFilter() {
    ...
    controller.searchDocumentFilter.getCheckModel().getCheckedItems()
  ...
  /**
   * Initializes the auto-complete functionality for the document search field. Displays suggestions
  ...
  /**
   * Updates the list of suggestions based on the user's query in the search field. Displays the
   * suggestions in a menu.
   *
      ...
      for (int j = 0; j < documentList.get(i).size(); j++) {
        ...
        Task<Void> loadController = new Task<>() {
          ...
          protected Void call() throws Exception {
            docContainerControllerList.get(row).get(col)
                .setDocData(documentList.get(row).get(col), null,
                    UserDocContainerController.HOME_DOCUMENT);
            return null;
          }
        };
  ...
  private void loadDocument(String query) {
    ...
    try {
      ...
      documentList.addAll(DocumentDAO.getInstance().searchAndGroupDocuments(query,
          controller.searchDocumentFilter.getCheckModel().getCheckedItems(), MAX_DOCUMENT_ROW,
          MAX_DOCUMENT_COL));
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  /**
   * Handles the event when the user clicks on their avatar image. Triggers a 3D rotation animation
   * for the avatar and related information.
   *
  ...
  /**
   * Handles the event when the user exits the avatar info area. Reverses the 3D rotation
   * animation.
   *
   * @param infoVBox The VBox containing additional information about the user.
   * @param pic      The avatar image view.
