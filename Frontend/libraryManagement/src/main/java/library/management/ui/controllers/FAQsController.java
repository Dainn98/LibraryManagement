/**
 * The FAQsController class is responsible for managing the Frequently Asked Questions (FAQ)
 * interface, including handling user input, loading questions and answers, and integrating
 * speech-to-text functionality. This controller supports two modes:
...
public class FAQsController implements properties {
  ...
  private void loadQuestion(GridPane gPane, ScrollPane faqSPane, JFXTextArea faqRequestContainer) {
    ...
    VBox userInputSection = createFAQsContainer(new Label("User:"), question, faqSPane, RIGHT);
    ...
    Task<VBox> getAnswer = new Task<>() {
      ...
      protected VBox call() {
        ...
        return createFAQsContainer(new Label("Response:"), answer, faqSPane, LEFT);
      }
    };
  ...
  private void initMicro(Recognizer recognizer, TargetDataLine microphone,
    ...
    while (!SpeechToText.stopRecognition) {
      ...
      if (recognizer.acceptWaveForm(buffer, bytesRead)) {
        ...
        if (!text.isEmpty()) {
          faqRequestContainer.setText(faqRequestContainer.getText() + " " + text);
