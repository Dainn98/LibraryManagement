...
/**
 * The {@code ProcessingController} is responsible for managing the processing
 * of loan requests for a user. It handles the display and management of
 * documents that are currently pending to be processed.
 * <p>
 * This class interacts with the LoanDAO to fetch pending loans and displays
 * the related document information in the UI. The documents are displayed
 * in a grid layout and are updated asynchronously to improve performance.
 */
public class ProcessingController extends UserSubController implements properties {
    ...
    /**
     * Loads the list of pending documents that need to be processed by the user.
     * It fetches pending loans for the logged-in user, creates UI elements
     * for each document, and asynchronously loads the document data into the UI.
     */
