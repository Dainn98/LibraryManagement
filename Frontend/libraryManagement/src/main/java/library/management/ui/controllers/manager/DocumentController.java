 ...
 * within the application's user interface. It extends the functionality of ManagerSubController and
 * integrates with a MainController instance to manipulate and present document data.
 * <p>
 * The class provides initialization of document view elements, loading and searching of document
 * data, management of selection checkboxes for batch operations, and facilitating document
 * deletion.
 */
public class DocumentController extends ManagerSubController {

  /**
   * A list of Document objects that can be observed for changes. This list is used to store and
   * manage documents within the DocumentController. It can be monitored by UI components for any
   * modifications to dynamically update the user interface.
   */
   ...
   * <p>
   * This list is used to maintain an observable collection of checkbox states, allowing the GUI
   * elements to react to changes and update accordingly. Typically associated with a user interface
   * where multiple checkboxes need to be monitored for changes in their states.
   * <p>
   * The ObservableList ensures that any modifications to the list, such as adding or removing
   * elements, will be automatically reflected
   */
  ...
  /**
   * Loads document data into the controller's view. This method clears the current list of
   * documents and repopulates it by fetching a list of books from the DocumentDAO singleton
   * instance. It then sets the updated list to the document view's items. Additionally, it
   * initializes the checkbox elements in the view by calling the initializeCheckBox method.
   */
  ...
  /**
   * Handles the search functionality for the document text field within the document view. This
   * method clears the current list of documents and updates it with the search results obtained
   * from the DocumentDAO. It fetches the search term entered by the user in the document text
   * field, processes the search by converting the term to lowercase, and updates the document view
   * with the filtered list of documents. Additionally, it initializes the checkboxes related to the
   * documents in the view. This ensures that the document view reflects the current search query
   * and displays
   */
   ...
   * SimpleBooleanProperty instances set to false. It also adds a listener to each BooleanProperty
   * that ensures if any checkbox is deselected, a master checkbox in the controller is also
   * deselected.
   **/
  ...
  /**
   * Updates the check box status for all documents based on the selection state of the master check
   * box in the view. If the master check box is selected, all individual document check boxes will
   * be set to selected, and vice versa.
   */
  ...
  /**
   * Handles the deletion of documents when the delete hyperlink is activated. This method prompts
   * the user with a confirmation alert. If the user confirms the deletion, it iterates over a list
   * of documents and deletes those that are selected (as indicated by the corresponding checkbox
   * status). Post-deletion, the document data is reloaded to reflect any changes.
   */
   ...
   * <p>
   * This method retrieves the document currently selected by the user and prompts a confirmation
   * alert to ensure the user intends to delete the document. If confirmed, the document is removed
   * from the data store via the DocumentDAO. After deletion, the document data is reloaded to
